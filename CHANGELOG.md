...
## [0.6.4] — 2026-09-24 🔕 管理員通知合併節流

### ✨ 改良
- **失敗／警示通知節流**：同一種問題（標題 + 錯誤類型／訊息，數字正規化後比對）10 分鐘內只推第一則，其餘合併計數；視窗過後再發生時推播並附「前 10 分鐘內另有 N 則相同通知已合併」。跨使用者、跨 instance 共用（Firestore `dmg_notify_throttle`，24 小時 TTL 自動清除）
- **移除重複通知**：
  - 後端「開始產生會議摘要」（約 10 秒後就有成功／失敗通知）；照片數併入成功通知
  - 前端「User completed service」（與後端「會議摘要產出成功」重複）
  - 摘要失敗時，後端已通知的情況前端不再重複回報
  - 摘要產出空白時只發「警示」，不再另發「失敗」
- 一份報告的通知從約 5～6 則降為約 3 則（開始使用、摘要成功、匯出）
...
### 🔒 資安
- **每小時使用上限**（Firestore 計數，`dmg_rate_limits` + TTL 自動清除）：以 App Check token（單一使用者）為主、IP 為寬鬆上限（學校常整校共用 IP）
  | 功能 | 每人（token） | 每 IP |
  |---|---|---|
  | 照片描述 | 40 | 300 |
  | 會議摘要 | 10 | 60 |
  | 取得驗證憑證 | — | 60 |
  | 前端事件 / 匯出通知 | 30 / 20 | 150 / 100 |
- 只存 SHA-256 雜湊，不保存原始 IP 或 token；IP 取 `X-Forwarded-For` 最後一段，偽造 header 無法繞過
- 超過上限時顯示「使用次數已達每小時上限，請稍後再試」，不再重試
- 啟用 GitHub Dependabot 安全警示與自動修補 PR

### ⬆️ 升級
- Next.js 15.2.3 → 15.5.26；前端 npm audit：critical 3 → 0、high 23 → 9

---

## [0.6.1] — 2026-09-24 🛡️ 人機驗證體驗修正

### 🐛 修正
- Turnstile 需要互動時改為**畫面中央卡片 + 半透明背景 + 說明文字**，不再藏在底部被浮動按鈕蓋住
- 驗證失敗 / 未完成時照片顯示明確原因（「人機驗證未完成，請按『再試一次』…」），不再只寫「產出失敗」
- `unauthenticated` / `invalid-argument` 不再白白重試三輪（原本最長會卡 6 分鐘）
- 修正 Turnstile 失敗後拋出 `Nothing to reset found` 未處理例外（`retry: 'never'` + error-callback 回傳 true + 延後移除 widget）
